...
if [[ -z $USERNAME_AVAIL ]]
then
  # Insert new user into the database if not already present
...
# Initialize the secret number to guess
...
do
  ...
  then
    ...
    # Check if the guess is correct or provide hints
  ...
  GUESS=$((GUESS + 1))  # Increment the guess count
done
